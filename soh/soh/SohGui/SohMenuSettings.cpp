...
void SohMenu::AddMenuSettings() {
    ...
    AddWidget(path, "Back Up Saves Now", WIDGET_BUTTON)
        ...
        .Callback([](WidgetInfo& info) {
            ...
            std::error_code ec;
            const fs::path docs = Ship::Context::GetRawInstance()->GetAppDirectoryPath();
            char stamp[32];
            ...
            bool ok = !ec;
            if (ok && fs::exists(docs / "Save")) {
                fs::copy(docs / "Save", dst / "Save",
                         fs::copy_options::recursive | fs::copy_options::overwrite_existing, ec);
                ok = ok && !ec;
            }
            if (ok && fs::exists(docs / "shipofharkinian.json")) {
                fs::copy_file(docs / "shipofharkinian.json", dst / "shipofharkinian.json",
    ...
    AddWidget(path, "Show D-Pad", WIDGET_CVAR_CHECKBOX)
        ...
        .RaceDisable(false)
        .Options(CheckboxOptions().Tooltip(
            "Adds a D-pad on the left side. Needed for D-pad enhancements like Equip Items on D-pad "
            "and ArrowCycle - without it those buttons cannot be pressed by touch."));
